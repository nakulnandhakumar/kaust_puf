...
output layer neurons of the model automatically. The value after applying the sigmoid function represents
the probability of the input belonging to the class reresented by that neuron. nn.CrossEntropyLoss() is
a loss function that applies a softmax function automically to the output and is better for multi-class
classification problems. BCE is better for binary classification problems.
'''
...
Y1 = np.ones(len(X1)).astype(np.float32)
...
Y6 = np.zeros(len(X6)).astype(np.float32)
...
X8_tensor = torch.tensor(X8).to(device)
...
print(f"Accuracy on test set Device 1: {accuracy_test:.2f}%")